...
default_cfgs = {
    # patch model_data (my experiments)
    'vit_small_patch16_224': _cfg(
        url='https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-weights/vit_small_p16_224-15ec54c9.pth',
    ),

    # patch model_data (weights ported from official Google JAX impl)
    ...
    # patch model_data, imagenet21k (weights ported from official Google JAX impl)
...
class VisionTransformer(torch.nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        Args:
            ...
            distilled (bool): model includes a distillation token and head as in DeiT model_data
        ...
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other model_data
    ...
    def reset_classifier(self, num_classes, global_pool=''):
        self.num_classes = num_classes
        self.head = torch.nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else torch.nn.Identity()
    ...
    * When called w/ valid n (module name) and jax_impl=True, will (hopefully) match JAX impl
    ...
    if isinstance(m, torch.nn.Linear):
        if n.startswith('head'):
            ...
            torch.nn.init.constant_(m.bias, head_bias)
            ...
            if jax_impl:
                ...
                if m.bias is not None:
                    torch.nn.init.zeros_(m.bias)
    elif jax_impl and isinstance(m, torch.nn.Conv2d):
...
def resize_pos_embed(posemb, posemb_new, num_tokens=1, gs_new=()):
    ...
    posemb_grid = torch.torch.nn.functional.interpolate(posemb_grid, size=gs_new, mode='bilinear')
...
def checkpoint_filter_fn(state_dict, model):
    ...
    for k, v in state_dict.items():
        if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
            # For old model_data that I trained prior to conv based patchification
...
def _create_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer model_data.')

    # NOTE this extra code to support handling of repr size for in21k pretrained model_data
...
def vit_small_patch16_224(pretrained=False, **kwargs):
    ...
    NOTE:
        ...
        * this model does not have a bias for QKV (unlike the official ViT and DeiT model_data)
    """
    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=8, num_heads=8, mlp_ratio=3.,
        qkv_bias=False, norm_layer=torch.nn.LayerNorm, **kwargs)
    if pretrained:
